...
let deleteUsersByAwardId = awardId => {
  const sql = `delete from award_user where award_id = ?`
  return queryHelper.queryPromise(sql, awardId)
}
...
let studentAwardCount = (userId, filter) => {
  const sql = `select count(*) as number from (select user.user_name as username,project.project_name as projectName,award.award_name as awardName, award.award_identity as awardCategory,award.award_level as awardLevel, award.award_time as awardTime from award_user left join award on award_user.award_id=award.award_id left join user on award_user.user_id=user.user_id left join project on project.project_id=award_user.award_project where award_user.user_id='${userId}') as t  ${filter ? 'where ' + filter : ''}`
  return queryHelper.queryPromise(sql)
}
